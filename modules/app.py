"""C'est un module qui crée une interface graphique pour simuler un vote électronique"""
...
class App(tk.Tk):
    ...
    #liste
    LISTE_EURO = ("Abstention", "l'Union Populaire Républicaine", "la France Insoumise",\
      "les Républicains", "Debout la France", "Le Rassemblement National",\
      "Place Publique - Parti Socialiste", "Europe Ecologie les Verts",\
      "La République en Marche", "Le Parti Communiste",\
      "l'Union des Démocrates Indépendants", "Génération.s", "les Patriotes")

    def __init__(self):
        """
        initie la fen principale (self) puis les composant de la première fenêtre
        """
        tk.Tk.__init__(self)
        ...
        #1ere entry
        self.entry_pseudo = tk.Entry(self, textvariable="", width=25, \
        ...
        #bouton
        self.button_valider = tk.Button(self, cursor="dotbox", text="Valider",\
         comman=self.valider, width=30, font=self.BUTTON_FONT)
    ...
    def compo_fen2(self):
        """Cette fonction fait apparaître la deuxième fenêtre"""
        ...
        #spinbox
        self.spinb_euro = tk.Spinbox(self, value=self.LISTE_EURO,\
         justify="center", wrap=True, width=50, font=self.ENTRY_FONT)
        self.spinb_euro.place(x=40, y=130)

        #bouton
        self.button_valider_euro = tk.Button(self, cursor="dotbox", text="Valider",\
         command=self.valider_euro, width=30, font=self.BUTTON_FONT)
        self.button_valider_euro.place(x=50, y=200)


        # self.liste_partis.pack()

    def dest_composants(self, liste):
        """
    ...
    def valider_euro(self):
        ...
        self.dest_composants([self.spinb_euro, self.button_valider_euro])
    ...
    def valider(self):
        ...
        if user.verif_idtf():
            ...
            self.dest_composants([self.label_titel, self.entry_pseudo, self.entry_mdp, self.button_valider])
            self.compo_fen2()
        else:
